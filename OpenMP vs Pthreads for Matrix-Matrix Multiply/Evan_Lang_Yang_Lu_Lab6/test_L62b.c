/*****************************************************************************/
// gcc -O1 test_L62b.c -lrt -o test_L62b
// OMP_NUM_THREADS=20 ./test_L62b
...
void mmm_ijk_openMP(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_kij(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_jki(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_ijk_opt_threaded(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_ijk_omp(matrix_ptr a, matrix_ptr b, matrix_ptr c);
void mmm_ijk_openMP_naive(matrix_ptr a, matrix_ptr b, matrix_ptr c);
...
int main(int argc, char *argv[])
{
  ...
  */
  ...
  printf("row_len, blocked_ijk_time, openMP_blocked_ijk_time_middle, openMP_blocked_ijk_time_outer\n");
  {
    int i, j;
...
void mmm_ijk_opt_threaded(matrix_ptr a, matrix_ptr b, matrix_ptr c)
{
  ...
  #pragma omp parallel shared(a0, b0, c0, en, length) private(i,j,k,ii,jj,kk,sum)
  #pragma for collapse(3)
  for (i=0; i<en; i+=bsize)
  {
    for (j=0; j<en; j+=bsize)
    {
      for (k=0; k<en; k+=bsize)
      {
          ...
          for (jj=j;jj<j+bsize;jj++)
            {
              ...
              for (kk=k;kk<k+bsize;kk++)
                {
                  sum += a0[ii*length+kk] * b0[kk*length+jj];
                }
              c0[ii*length+jj] += sum;
            }
        }
...
//STANDARD OMP FROM YOUR CODE
/*void mmm_ijk_omp(matrix_ptr a, matrix_ptr b, matrix_ptr c)
{
  long int i, j, k;
  long int row_length = get_matrix_row_length(a);
  data_t *a0 = get_matrix_start(a);
  data_t *b0 = get_matrix_start(b);
  data_t *c0 = get_matrix_start(c);
  data_t sum;
