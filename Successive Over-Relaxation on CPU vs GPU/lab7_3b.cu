...
int main(int argc, char *argv[])
{
 ...
 //***********************************************************
  ...
  gpu_SOR<<<GRID, BLOCKS>>>(d_arrptr , d_iterations, alloc_size);
  }*/
...
{
  int i = (blockIdx.y +threadIdx.y*gridDim.y); //row
  int j = (blockIdx.x+threadIdx.x*gridDim.x); //col
  data_t temp;
  int iters = 0;
  //boundary check
       if(i > 1 && j > 1 && i < rowlen-1 && j < rowlen-1){
           for(iters = 0; iters<2000/ITERS; iters++){ 
                     temp =  0.2 * ( data[(i)*rowlen+j] +
                                          data[(i)*rowlen+(j-1)] +
                                          data[(i-1)*rowlen+j] +
                                          data[i*rowlen+(j+1)]   +
                                          data[(i+1)*rowlen+j]);
                __syncthreads();
                data[i*rowlen+j] = temp;
            }
        } 
    //__syncthreads();
  }
