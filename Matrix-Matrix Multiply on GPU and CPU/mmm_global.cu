...
int main(int argc, char *argv[])
{
  ...
  dim3 BLOCK(16, 16, 1);
  dim3 GRID((alloc_size+16-1)/16, (alloc_size+16-1)/16, 1);
  OPTION++;//GPU
  for (x=0; x<NUM_TESTS && (n = A*x*x + B*x + C, n<=alloc_size); x++) {
    clock_gettime(CLOCK_PROCESS_CPUTIME_ID, &time_start);
...
__global__ void gpu_MMM(data_t *da0, data_t *db0, data_t *dc0, long int rowlen)
{
   ...
   long int i;
   if(row < rowlen && col<rowlen){
       data_t sum = 0;
       for(i = 0; i < rowlen; i++){
           sum += da0[row*rowlen+i] * db0[i*rowlen+col];
       }
       dc0[row*rowlen+col] = sum;
   }
}
